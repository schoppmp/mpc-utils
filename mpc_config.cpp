...
mpc_config::mpc_config() : config() {
  ...
  this->add_options()("party,p", po::value(&party_id)->required(),
                      "Party ID of this process");

  po::options_description desc_server(
      "Server options. These can be passed "
...
void mpc_config::validate() {
  ...
  if (server_hosts.size() != server_ports.size()) {
    BOOST_THROW_EXCEPTION(
        po::error("'server.host' and 'server.port' have to be"
