...
inline void save(
	...
	for(size_t k=0; k<mat.outerSize(); k++) {
	  for(typename Eigen::SparseMatrix<T, Ops_, StorageIndex_>::InnerIterator
			it(mat,k); it; ++it
		) {
			triplets.push_back(Eigen::Triplet<T, StorageIndex_>(it.row(), it.col(),
				it.value()));
	  }
	}
	ar & triplets;
