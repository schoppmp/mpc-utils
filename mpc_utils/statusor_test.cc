...
TEST(StatusOr, TestNoDefaultConstructorInitialization) {
  ...
  StatusOr<NoDefaultConstructor> statusor(Status(mpc_utils::error::CANCELLED, ""));
  EXPECT_FALSE(statusor.ok());
  EXPECT_EQ(statusor.status().code(), mpc_utils::error::CANCELLED);
  ...
  EXPECT_EQ(statusor2.status().code(), mpc_utils::error::UNKNOWN);
}
...
TEST(StatusOr, TestMoveOnlyStatusCtr) {
  StatusOr<std::unique_ptr<int>> thing(Status(mpc_utils::error::CANCELLED, ""));
...
TEST(StatusOr, TestMoveOnlyVector) {
  ...
  EXPECT_EQ(mpc_utils::error::UNKNOWN, another_vec[1].status().code());
}
...
TEST(StatusOr, TestMoveWithValuesAndErrors) {
  ...
  StatusOr<std::string> error1(Status(mpc_utils::error::UNKNOWN, "error1"));
  StatusOr<std::string> error2(Status(mpc_utils::error::UNKNOWN, "error2"));
...
TEST(StatusOr, TestDefaultCtor) {
  ...
  EXPECT_EQ(thing.status().code(), mpc_utils::error::UNKNOWN);
}
...
TEST(StatusOr, TestStatusCtor) {
  StatusOr<int> thing(Status(mpc_utils::error::CANCELLED, ""));
  EXPECT_FALSE(thing.ok());
  EXPECT_EQ(thing.status().code(), mpc_utils::error::CANCELLED);
}
...
TEST(StatusOr, TestCopyCtorStatusNotOk) {
  StatusOr<int> original(Status(mpc_utils::error::CANCELLED, ""));
...
TEST(StatusOr, TestAssignmentStatusNotOk) {
  StatusOr<int> source(Status(mpc_utils::error::CANCELLED, ""));
  StatusOr<int> target;
  target = source;
  EXPECT_EQ(target.status(), source.status());
}
...
TEST(StatusOr, TestStatus) {
  StatusOr<int> good(4);
  EXPECT_TRUE(good.ok());
  StatusOr<int> bad(Status(mpc_utils::error::CANCELLED, ""));
  EXPECT_FALSE(bad.ok());
  EXPECT_EQ(bad.status(), Status(mpc_utils::error::CANCELLED, ""));
}
...
TEST(StatusOrDeathTest, TestValueNotOk) {
  StatusOr<int> thing(Status(mpc_utils::error::CANCELLED, "cancelled"));
  EXPECT_DEATH(thing.ValueOrDie(), "cancelled");
}

TEST(StatusOrDeathTest, TestValueNotOkConst) {
  const StatusOr<int> thing(Status(mpc_utils::error::UNKNOWN, ""));
  EXPECT_DEATH(thing.ValueOrDie(), "");
}
...
TEST(StatusOr, TestPointerStatusCtor) {
  StatusOr<int*> thing(Status(mpc_utils::error::CANCELLED, ""));
  EXPECT_FALSE(thing.ok());
  EXPECT_EQ(thing.status(), Status(mpc_utils::error::CANCELLED, ""));
}
...
TEST(StatusOr, TestPointerCopyCtorStatusNotOkConverting) {
  StatusOr<Derived*> original(Status(mpc_utils::error::CANCELLED, ""));
  StatusOr<Base2*> copy(original);
  EXPECT_EQ(copy.status(), original.status());
}
...
TEST(StatusOr, TestPointerStatus) {
  ...
  EXPECT_TRUE(good.ok());
  StatusOr<const int*> bad(Status(mpc_utils::error::CANCELLED, ""));
  EXPECT_EQ(bad.status(), Status(mpc_utils::error::CANCELLED, ""));
}
...
TEST(StatusOrDeathTest, TestPointerValueNotOkConst) {
  const StatusOr<int*> thing(Status(mpc_utils::error::CANCELLED, "cancelled"));
  EXPECT_DEATH(thing.ValueOrDie(), "cancelled");
}
